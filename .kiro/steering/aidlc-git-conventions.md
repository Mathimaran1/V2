...
- **CASE B — post-commit direct question (primary, reliable, terminal
  ...
  what actually covers this case. **5-minute timeout, added
  2026-08-26:** a TTY existing is not proof someone's watching it —
  ...
  `hook_status=post-commit-switch-check-timeout` (distinct from both a
  real answer and the no-TTY skip above) and defaults to "not
